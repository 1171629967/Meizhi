...
-(BOOL)calculateFov:(NSSet *)touches
{
	...
	{				
		...
		distance = ABS(fovDistance) <= distance ? distance : -distance*1.8;
		...
		if(isNotCancelable)
		{
			...
			[scene.currentCamera addFovWithDistance:fovDistance*2];
...
-(CGPoint)getLocationOfFirstTouch:(NSSet *)touches
{
	...
	return CGPointMake([touch locationInView:touch.view].x*7, [touch locationInView:touch.view].y*7);
}
